<script lang="ts">
  ...
  const predefined: Snippet[] = [
    ...
    { id: "pre_3", name: "Sök på namn", sql: "SELECT orgnr, orgnamn, telefon, email, webbadress, postort\nFROM bolag\nWHERE telefon IS NOT NULL AND email IS NOT NULL AND webbadress IS NOT NULL\n  AND orgnamn LIKE '%Bilverkstad%'\nLIMIT 100" },
    ...
    { id: "pre_11", name: "100 Frisörer", sql: "SELECT orgnr, orgnamn, sni_1, sni_1_namn, postort\nFROM bolag\nWHERE sni_1 = '96210'\nLIMIT 100" },
    { id: "pre_12", name: "100 Taxibolag", sql: "SELECT orgnr, orgnamn, sni_1, sni_1_namn, postort\nFROM bolag\nWHERE orgnamn LIKE '%Taxi%' AND sni_1_namn != ''\nLIMIT 100" },
